...
class SlaDashboardController extends Controller
{
    private const MODULES = [
        ...
        'urgencias' => ['label' => 'Urgencias', 'event_type' => 'triage', 'color' => '#3B82F6', 'outlier_color' => '#EF4444', 'icon' => 'fa-truck-medical'],
    ...
    public function index(Request $request)
    {
        ...
        // 1. DATOS DEL MÓDULO ACTIVO (Scatter, Histograma, Boxplot)
        ...
        // 2. DATOS PARA HISTOGRAMA (Distribución por rangos)
        $ranges = ['0-30' => 0, '31-60' => 0, '61-90' => 0, '91-120' => 0, '121-150' => 0, '151+ (Anomalía)' => 0];
        foreach ($logs as $log) {
            $d = $log->duration_minutes;
            if ($d <= 30) $ranges['0-30']++;
            elseif ($d <= 60) $ranges['31-60']++;
            elseif ($d <= 90) $ranges['61-90']++;
            elseif ($d <= 120) $ranges['91-120']++;
            elseif ($d <= 150) $ranges['121-150']++;
            else $ranges['151+ (Anomalía)']++;
        }

        // 3. DATOS PARA BOXPLOT (Percentiles)
        $durations = $logs->pluck('duration_minutes')->filter()->sort()->values()->toArray();
        $boxplotData = $this->calculateBoxplot($durations);

        // 4. DATOS PARA BARRAS HORIZONTALES (Comparación de Módulos)
        $barData = [];
        ...
        // 5. TÍTULOS DESCRIPTIVOS (Regla de claridad)
