...
<div style="background:#F0FDF4; padding:3rem; border-radius:12px; text-align:center; border:2px solid #BBF7D0;">
    ...
    <h3 style="font-weight:800; color:#166534;">Sin alertas pendientes</h3>
    <p style="color:#4ADE80;">Todos los pacientes estan estables.</p>
</div>
@else
@foreach($alerts as $alert)
<div style="background:white; padding:1.25rem; border-radius:12px; box-shadow:0 4px 6px rgba(0,0,0,0.04); margin-bottom:1rem; border-left:5px solid {{ $alert->is_read ? '#94A3B8' : ($alert->severity === 'Crítica' ? '#DC2626' : '#F59E0B') }}; {{ !$alert->is_read ? 'animation: fadeIn 0.3s;' : 'opacity:0.7;' }}">
    <div style="display:flex; justify-content:space-between; align-items:flex-start;">
        <div style="flex:1;">
            <div style="display:flex; align-items:center; gap:0.5rem; margin-bottom:0.5rem;">
                @if(!$alert->is_read)<i class="fas fa-exclamation-circle" style="color:{{ $alert->severity === 'Crítica' ? '#DC2626' : '#F59E0B' }};"></i>@endif
                <span style="font-weight:800; color:{{ $alert->severity === 'Crítica' ? '#DC2626' : '#F59E0B' }}; font-size:0.9rem;">{{ $alert->type }}</span>
            ...
            <p style="font-size:0.9rem; color:#1E1A17; margin-bottom:0.5rem;">{{ $alert->message }}</p>
            @if($alert->triage)
        ...
        <form method="POST" action="{{ route('enfermeria.markAlertRead', $alert->id) }}">
            @csrf
            <button type="submit" style="background:#3B82F6; color:white; border:none; padding:0.5rem 1rem; border-radius:8px; font-weight:700; cursor:pointer; font-size:0.8rem;"><i class="fas fa-check"></i> Leido</button>
        </form>
        @endif
    </div>
</div>
