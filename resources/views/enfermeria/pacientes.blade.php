...
<div style="background:#F0FDF4; border:2px solid #86EFAC; border-radius:12px; padding:1rem 1.25rem; margin-bottom:1.5rem; display:flex; align-items:center; gap:0.75rem;">
    <i class="fas fa-check-circle" style="color:#2D9E6A; font-size:1.2rem;"></i>
...
<div style="background:white; padding:1.5rem; border-radius:12px; box-shadow:0 4px 6px rgba(0,0,0,0.04); margin-bottom:1.5rem; display:flex; justify-content:space-between; align-items:center;">
    <h3 style="font-weight:800;"><i class="fas fa-users" style="color:#3B82F6;"></i> Todos los Pacientes</h3>
    <span style="background:#EFF6FF; color:#1D4ED8; padding:0.3rem 0.8rem; border-radius:20px; font-size:0.8rem; font-weight:700;">{{ $patients->count() }} registrados</span>
</div>

<div style="background:white; border-radius:12px; box-shadow:0 4px 6px rgba(0,0,0,0.04); overflow:hidden;">
    <table style="width:100%; border-collapse:collapse;">
        <thead><tr style="background:#1E1A17; color:white; text-align:left;">
        ...
        <tr style="border-bottom:1px solid #E2E8F0; @if($p->triage_level === 'Rojo') background:#FEF2F2; @endif">
            ...
            <td style="padding:0.75rem;">
                @if($p->assigned_area)
                <span style="background:#EFF6FF; color:#1D4ED8; padding:0.15rem 0.5rem; border-radius:8px; font-size:0.75rem; font-weight:700;">{{ $p->assigned_area }}</span>
                @else
                <span style="color:#94A3B8; font-size:0.75rem;">Sin asignar</span>
                @endif
            </td>
            <td style="padding:0.75rem;">
                @php $statusColors = ['En Espera'=>'#F59E0B','En Atención'=>'#3B82F6','Hospitalizado'=>'#8B5CF6','Dado de Alta'=>'#2D9E6A','Derivado'=>'#64748B']; @endphp
            ...
            <td style="padding:0.75rem;">
                ...
                <div style="display:flex; gap:4px; flex-wrap:wrap;">
                    ...
                    <form method="POST" action="{{ route('enfermeria.enviarA', $p->id) }}" style="display:flex; gap:3px;">
                        ...
                        <button type="submit" style="background:#3B82F6; color:white; border:none; padding:0.25rem 0.5rem; border-radius:4px; font-size:0.65rem; cursor:pointer; font-weight:700;"><i class="fas fa-paper-plane"></i> Enviar</button>
                    </form>
                    @else
                    <form method="POST" action="{{ route('enfermeria.reasignar', $p->id) }}" style="display:flex; gap:3px;">
                        ...
                        <button type="submit" style="background:#F59E0B; color:white; border:none; padding:0.25rem 0.5rem; border-radius:4px; font-size:0.65rem; cursor:pointer; font-weight:700;"><i class="fas fa-exchange-alt"></i> Cambiar</button>
                    </form>
                    @endif
                    <form method="POST" action="{{ route('enfermeria.darAlta', $p->id) }}">
                        @csrf @method('PUT')
                        <button type="submit" style="background:#2D9E6A; color:white; border:none; padding:0.25rem 0.5rem; border-radius:4px; font-size:0.65rem; cursor:pointer; font-weight:700;" onclick="return confirm('Dar de alta a {{ $p->patient_name }}?')"><i class="fas fa-check"></i> Alta</button>
                    </form>
                </div>
                @else
