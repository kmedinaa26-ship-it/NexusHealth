...
<div style="background:white; padding:1.5rem; border-radius:12px; box-shadow:0 4px 6px rgba(0,0,0,0.04); margin-bottom:1.5rem;">
    <h3 style="font-weight:800;"><i class="fas fa-pills" style="color:#3B82F6;"></i> Medicamentos Autorizados para Enfermeria</h3>
    <p style="color:#64748B; font-size:0.85rem;">Solo puedes administrar los medicamentos marcados para tu perfil.</p>
</div>
<div style="display:grid; grid-template-columns:repeat(3,1fr); gap:1rem;">
@foreach($meds as $med)
<div style="background:white; padding:1.25rem; border-radius:12px; box-shadow:0 4px 6px rgba(0,0,0,0.04); border-top:3px solid #3B82F6;">
    ...
    <div style="display:flex; justify-content:space-between; font-size:0.8rem;">
        <span style="font-weight:700; color:{{ $med->stock <= $med->min_stock ? '#DC2626' : '#2D9E6A' }}">Stock: {{ $med->stock }}</span>
        <span style="background:#EFF6FF; color:#1D4ED8; padding:0.1rem 0.4rem; border-radius:6px; font-size:0.7rem; font-weight:700;">{{ $med->origin }}</span>
    </div>
