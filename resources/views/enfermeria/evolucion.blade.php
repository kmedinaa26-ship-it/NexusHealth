...
@section('nav-evolucion', 'active')

@section('content')
<div style="display:grid; grid-template-columns:1fr 1fr; gap:1.5rem;">
    <!-- Nueva Nota -->
    <div style="background:white; padding:1.5rem; border-radius:12px; box-shadow:0 4px 6px rgba(0,0,0,0.04);">
        <h3 style="font-weight:800; margin-bottom:1.25rem;"><i class="fas fa-notes-medical" style="color:#3B82F6;"></i> Nota de Evolucion</h3>
        <form method="POST" action="{{ route('enfermeria.storeEvolution') }}">
            ...
            <button type="submit" style="width:100%; background:#3B82F6; color:white; border:none; padding:0.75rem; border-radius:8px; font-weight:700; cursor:pointer;"><i class="fas fa-save"></i> Guardar Nota</button>
        </form>
    </div>
    ...
    <div style="background:white; padding:1.5rem; border-radius:12px; box-shadow:0 4px 6px rgba(0,0,0,0.04);">
        ...
        <div style="border-left:3px solid {{ $e->priority === 'Urgente' ? '#DC2626' : '#3B82F6' }}; padding:0.75rem; margin-bottom:0.75rem; background:{{ $e->alert_doctor ? '#FEF2F2' : '#FAFAFA' }}; border-radius:0 8px 8px 0;">
            ...
            <div style="display:flex; gap:0.5rem; margin-top:0.25rem;">
                <span style="background:{{ $e->priority === 'Urgente' ? '#FEF2F2' : '#EFF6FF' }}; color:{{ $e->priority === 'Urgente' ? '#DC2626' : '#3B82F6' }}; padding:0.1rem 0.4rem; border-radius:4px; font-size:0.65rem; font-weight:700;">{{ $e->priority }}</span>
