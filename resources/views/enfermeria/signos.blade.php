...
<div style="display:grid; grid-template-columns:1fr 1fr; gap:1.5rem;">
    ...
    <div style="background:white; padding:1.5rem; border-radius:12px; box-shadow:0 4px 6px rgba(0,0,0,0.04);">
        <h3 style="font-weight:800; margin-bottom:1.25rem;"><i class="fas fa-stethoscope" style="color:#3B82F6;"></i> Registrar Signos Vitales</h3>
        <form method="POST" action="{{ route('enfermeria.storeVitals') }}">
            ...
            <button type="submit" style="width:100%; background:#3B82F6; color:white; border:none; padding:0.75rem; border-radius:8px; font-weight:700; cursor:pointer;"><i class="fas fa-save"></i> Guardar Signos Vitales</button>
        </form>
    </div>
    ...
    <div style="background:white; padding:1.5rem; border-radius:12px; box-shadow:0 4px 6px rgba(0,0,0,0.04);">
        ...
        <div style="border-left:3px solid {{ $v->is_critical ? '#DC2626' : '#3B82F6' }}; padding:0.75rem; margin-bottom:0.75rem; background:{{ $v->is_critical ? '#FEF2F2' : '#FAFAFA' }}; border-radius:0 8px 8px 0;">
