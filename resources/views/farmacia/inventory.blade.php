...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
    <h3 style="font-weight: 800; color: #1E1A17;">Inventario Farmaceutico Completo</h3>
    <button onclick="document.getElementById('modal-add').style.display='flex'" style="background:#2D9E6A; color:white; border:none; padding:0.6rem 1.2rem; border-radius:8px; font-weight:700; cursor:pointer;">
...
<div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-bottom: 1.5rem;">
    @php $levels = ['A' => ['color'=>'#C7291C','label'=>'Especialista'], 'B' => ['color'=>'#FF8C42','label'=>'Hospitalizacion'], 'C' => ['color'=>'#2D9E6A','label'=>'Basico'], 'Enfermera' => ['color'=>'#3B82F6','label'=>'Enfermeria']]; @endphp
    ...
    <div onclick="filterByLevel('{{ $level }}')" style="background: white; padding: 1.5rem; border-radius: 12px; border-top: 4px solid {{ $info['color'] }}; box-shadow: 0 4px 6px rgba(0,0,0,0.04); cursor: pointer; transition: 0.2s;" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform='none'">
        ...
        <div style="font-size:2rem; font-weight:800; color:#1E1A17; margin-top:0.5rem;">{{ $count }}</div>
    </div>
    @endforeach
</div>
        ...
        <tbody>
            ...
            <tr style="border-bottom: 1px solid #E5E7EB; {{ $med->isExpired() ? 'background:#FFF1F0;' : '' }}">
                ...
                <td style="padding:0.75rem;">
                    ...
                    <span style="background:#3B82F6; color:white; padding:0.15rem 0.4rem; border-radius:8px; font-size:0.7rem; font-weight:700;"><i class="fas fa-check"></i> SI</span>
                    @else
...
<div id="modal-add" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); z-index:100; align-items:center; justify-content:center;">
    <div style="background:white; padding:2rem; border-radius:12px; width:550px; max-height:90vh; overflow-y:auto;">
        <h3 style="font-weight:800; margin-bottom:1.5rem;"><i class="fas fa-pills" style="color:#2D9E6A;"></i> Registrar Medicamento</h3>
        <form action="{{ route('farmacia.storeMedication') }}" method="POST">
            ...
            <div style="background:#EFF6FF; padding:0.75rem; border-radius:8px; margin-bottom:1rem; border-left:3px solid #3B82F6;">
                <label style="display:flex; align-items:center; gap:0.5rem; cursor:pointer; font-weight:700; font-size:0.85rem; color:#1E3A8A;">
                    <input type="checkbox" name="enfermera_can_administer" id="enfermera-check" value="1" style="width:18px; height:18px;">
                    <i class="fas fa-user-nurse" style="color:#3B82F6;"></i> Enfermeria puede administrar este medicamento
                </label>
            ...
            <div style="display:flex; gap:10px;">
                ...
                <button type="submit" style="flex:1; padding:0.8rem; background:#2D9E6A; color:white; border:none; border-radius:8px; cursor:pointer; font-weight:700;"><i class="fas fa-save"></i> Registrar</button>
            </div>
        </form>
    </div>
</div>
