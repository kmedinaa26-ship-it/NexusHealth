...
<div style="background: white; padding: 1.5rem; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.04); margin-bottom: 1.5rem;">
    <a href="{{ route('farmacia.desabasto') }}" style="color:#2D9E6A; text-decoration:none; font-weight:700; font-size:0.85rem;"><i class="fas fa-arrow-left"></i> Volver a Desabasto</a>
    <h3 style="font-weight: 800; color: #1E1A17; margin-top: 0.5rem;">
        <i class="fas fa-exchange-alt" style="color:#3B82F6;"></i> Alternativas para: {{ $med->name }}
    </h3>
...
<div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.5rem;">
    ...
    <div style="background: white; padding: 1.5rem; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.04); border-left: 4px solid {{ $alt->alternative->stock > 0 ? '#2D9E6A' : '#C7291C' }};">
        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 1rem;">
            <div>
                <h4 style="font-weight: 800; color: #1E1A17;">{{ $alt->alternative->name }}</h4>
                <div style="font-size:0.8rem; color:#736860;">{{ $alt->alternative->active_ingredient }}</div>
            </div>
            @if($alt->alternative->stock > 0)
            <span style="background:#EBF9F2; color:#065F46; padding:0.3rem 0.8rem; border-radius:10px; font-size:0.75rem; font-weight:700;">DISPONIBLE</span>
            @else
        ...
        <div style="background:#F9FAFB; padding:0.75rem; border-radius:8px; margin-bottom:1rem;">
            ...
            <div style="font-size:0.85rem; color:#1E1A17;">{{ $alt->notes }}</div>
        </div>

        <div style="display:grid; grid-template-columns:1fr 1fr; gap:0.5rem; font-size:0.8rem; color:#736860;">
            <div><i class="fas fa-boxes-stacked"></i> Stock: <strong style="color:{{ $alt->alternative->stock > 0 ? '#2D9E6A' : '#C7291C' }}">{{ $alt->alternative->stock }}</strong></div>
...
@else
<div style="background: #EFF6FF; padding: 2rem; border-radius: 12px; text-align: center; color: #1E3A8A;">
