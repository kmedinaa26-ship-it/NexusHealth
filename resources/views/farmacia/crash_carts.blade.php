...
<div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.5rem;">
    ...
    @php $statusColors = ['Completo' => '#2D9E6A', 'Incompleto' => '#C7291C', 'En Uso' => '#FF8C42', 'Reabasteciendo' => '#3B82F6']; @endphp
    <div style="background: white; padding: 1.5rem; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.04); border-top: 4px solid {{ $statusColors[$cart->status] ?? '#736860' }};">
        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 1rem;">
            <div>
                <h4 style="font-weight: 800; color: #1E1A17;">{{ $cart->name }}</h4>
        ...
        <div style="background:#F9FAFB; padding:1rem; border-radius:8px; margin-bottom:1rem;">
            ...
            <div style="font-size:0.85rem; color:#1E1A17;">{{ $cart->contents }}</div>
        </div>

        <div style="display:flex; justify-content:space-between; align-items:center;">
            <div style="font-size:0.8rem; color:#736860;">
                @if($cart->last_checked)
                    <i class="fas fa-check-circle" style="color:#2D9E6A;"></i> Verificado: {{ \Carbon\Carbon::parse($cart->last_checked)->format('d/m/Y H:i') }} por {{ $cart->checked_by }}
                @else
            ...
            <form action="{{ route('farmacia.checkCart', $cart->id) }}" method="POST">
                @csrf
                <button type="submit" style="background:#2D9E6A; color:white; border:none; padding:0.5rem 1rem; border-radius:6px; font-weight:700; cursor:pointer; font-size:0.8rem;">
