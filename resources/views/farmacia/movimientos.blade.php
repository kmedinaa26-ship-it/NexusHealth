...
<div style="background: white; padding: 1.5rem; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.04); margin-bottom: 1.5rem; display: flex; justify-content: space-between; align-items: center;">
    <h3 style="font-weight: 800; color: #1E1A17;"><i class="fas fa-clipboard-list" style="color:#2D9E6A;"></i> Movimientos de Farmacia</h3>
    <span style="background:#EBF9F2; color:#1B7A4A; padding:0.3rem 0.6rem; border-radius:10px; font-size:0.75rem; font-weight:700;">Ultimos 50 registros</span>
</div>
        ...
        <tbody>
            ...
            <tr style="border-bottom: 1px solid #E5E7EB; {{ $log->action == 'Dispensacion DENEGADA' ? 'background:#FFF1F0;' : '' }}">
                ...
                <td style="padding:0.75rem;">
                    @php $actionColor = '#2D9E6A'; @endphp
                    @if(strpos($log->action, 'DENEGADA') !== false) @php $actionColor = '#C7291C'; @endphp @endif
                    @if(strpos($log->action, 'Registrado') !== false) @php $actionColor = '#3B82F6'; @endphp @endif
