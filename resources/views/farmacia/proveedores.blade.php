...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
    <h3 style="font-weight: 800; color: #1E1A17;"><i class="fas fa-truck" style="color:#2D9E6A;"></i> Proveedores y Score</h3>
    <a href="{{ route('farmacia.crearOrden') }}" style="background:#2D9E6A; color:white; text-decoration:none; padding:0.6rem 1.2rem; border-radius:8px; font-weight:700; font-size:0.85rem;"><i class="fas fa-plus"></i> Nueva Orden de Compra</a>
</div>

<div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.5rem;">
    @foreach($providers as $prov)
    @php $totalScore = ($prov->delivery_score + $prov->price_score + $prov->quality_score) / 3; @endphp
    <div style="background: white; padding: 1.5rem; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.04); border-left: 4px solid {{ $totalScore >= 8 ? '#2D9E6A' : ($totalScore >= 6 ? '#FF8C42' : '#C7291C') }};">
        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 1rem;">
            <div>
                <h4 style="font-weight: 800; color: #1E1A17;">{{ $prov->name }}</h4>
                <div style="font-size:0.8rem; color:#736860;">{{ $prov->supply_type }} | RFC: {{ $prov->rfc }}</div>
            </div>
            <div style="text-align:center;">
                <div style="font-size:1.8rem; font-weight:800; color:{{ $totalScore >= 8 ? '#2D9E6A' : ($totalScore >= 6 ? '#FF8C42' : '#C7291C') }};">{{ number_format($totalScore, 1) }}</div>
        ...
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.5rem; margin-bottom: 1rem;">
            <div style="background:#F9FAFB; padding:0.5rem; border-radius:6px; text-align:center;">
                ...
                <div style="font-weight:800; color:#1E1A17;">{{ $prov->delivery_score }}/10</div>
            </div>
            <div style="background:#F9FAFB; padding:0.5rem; border-radius:6px; text-align:center;">
                <div style="font-size:0.65rem; color:#736860; font-weight:700;">PRECIO</div>
                <div style="font-weight:800; color:#1E1A17;">{{ $prov->price_score }}/10</div>
            </div>
            <div style="background:#F9FAFB; padding:0.5rem; border-radius:6px; text-align:center;">
                <div style="font-size:0.65rem; color:#736860; font-weight:700;">CALIDAD</div>
                <div style="font-weight:800; color:#1E1A17;">{{ $prov->quality_score }}/10</div>
            </div>
        </div>
